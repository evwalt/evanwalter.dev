...
export default function App() {
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-y-10">
      ...
      {loading ? <Status addClassNames="text-gray-500">Loading your joke...</Status> : null}
      {success ? (
        <>
          <div className="relative">
            ...
            <JokeText addClassNames="min-[374px]:text-left min-[374px]:pl-20">{setup}</JokeText>
          </div>
          <Btn addClassNames="bg-pink-500 mx-auto" onClick={() => setShowPunchline((s) => !s)}>
      ...
      {showPunchline ? (
        <div className="relative">
          ...
          <JokeText addClassNames="min-[374px]:text-right min-[374px]:pr-20">{punchline}</JokeText>
        </div>
      ) : null}
...
export function Btn({ addClassNames, onClick, children }: BtnProps) {
  return (
    <button
      className={`${addClassNames} rounded-full px-6 py-4 text-white transition duration-300 hover:scale-105 hover:ease-in-out`}
      onClick={onClick}
    >
      {children}
    </button>
...
export function Status({ addClassNames = '', children }: StatusProps) {
  return <p className={`${addClassNames} text-center font-bold uppercase`}>{children}</p>;
}
...
export function JokeText({ addClassNames, children }: JokeTextProps) {
  return <p className={`${addClassNames} text-center text-xl min-[374px]:text-2xl`}>{children}</p>;
}
